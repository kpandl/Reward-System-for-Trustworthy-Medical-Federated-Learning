...
for i in range(num_jobs):
    ...
    with open("job_f_age_as_is_3_parallel_"+str(seeds_start+i*4)+".sh", "w+") as file:
        file.writelines(line_list)

    time.sleep(0.8)
    os.system("sbatch -p gpu-single --cpus-per-gpu=16 --time=24:00:00 --mem=384000 --gres=gpu:4 " + "job_f_age_as_is_3_parallel_"+str(seeds_start+i*4)+".sh")